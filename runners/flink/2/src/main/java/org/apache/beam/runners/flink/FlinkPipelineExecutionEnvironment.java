 ...
 * a Streaming or Batch processing one, it creates the adequate execution environment ({@link StreamExecutionEnvironment}), the necessary {@link
 * FlinkPipelineTranslator} ({@link
...
class FlinkPipelineExecutionEnvironment {
  ...
  public void translate(Pipeline pipeline) {
    ...
    this.flinkEnv = FlinkExecutionEnvironments.createStreamExecutionEnvironment(options);
    if (hasUnboundedOutput && !flinkEnv.getCheckpointConfig().isCheckpointingEnabled()) {
      LOG.warn("UnboundedSources present which rely on checkpointing, but checkpointing is disabled.");
    }
    translator = new FlinkStreamingPipelineTranslator(flinkEnv, options, options.isStreaming());
  ...
  public PipelineResult executePipeline() throws Exception {
    ...
    if (flinkEnv != null) {
      if (options.getAttachedMode()) {
        JobExecutionResult jobExecutionResult = flinkEnv.execute(jobName);
  ...
  private FlinkDetachedRunnerResult createDetachedPipelineResult(JobClient jobClient, FlinkPipelineOptions options) {
  ...
  private FlinkRunnerResult createAttachedPipelineResult(JobExecutionResult result) {
    ...
    MetricsPusher metricsPusher =
      new MetricsPusher(flinkRunnerResult.getMetricsContainerStepMap(), options.as(MetricsOptions.class),
        flinkRunnerResult);
    metricsPusher.start();
